...
(function () {
  ...
  var meta = document.querySelector('meta[name="hs-edit"]');
  if (!meta) return;
  ...
  try {
    ...
    if (m) { var v = decodeURIComponent(m[1]); if (v === 'off') sessionStorage.removeItem('hs-edit-key'); else sessionStorage.setItem('hs-edit-key', v); }
    key = sessionStorage.getItem('hs-edit-key') || '';
  } catch (e) { key = m ? decodeURIComponent(m[1]) : ''; }
  if (!key || key === 'off' || sha256(key) !== hash) return;

  /* ══ 편집 모드 ══ */
  var SKIP = '#gal,#pfGrid,.blist,#noticeList,#board,#fileList,#storyCards,#blogCount,#pwState,#power,.lb,#formDone,.quick,#es-editbar,#es-dlg,form,.pick,#cart,#tabs';
  ...
  function imgTarget(t) {
    ...
    var el = t.closest('img,[style*="background-image"],.hero .img'); if (!el || el.closest('.lb,#es-editbar')) return null;
  ...
  function setImage(src, d) {
    images[src] = d;
    [].slice.call(document.querySelectorAll('main img,main [style*="background-image"],.hero .img')).forEach(function (el) {
  ...
  function db(cb) {
    var rq = indexedDB.open('hs-edit', 1);
  ...
  if (document.querySelector('#hero')) { try { document.querySelector('#hero').dispatchEvent(new Event('pointerenter')); } catch (e) {} }
  ...
  function openDialog() {
    loadAll(function (all) {
      ...
      var saved = {}; try { saved = JSON.parse(localStorage.getItem('hs-edit-who') || '{}'); } catch (e) {}
      dlg.innerHTML = '<div class="box"><h3>수정 내용 보내기</h3><p class="d">바뀐 곳 <b>' + n + '</b>개' + (nImg ? ' (사진 ' + nImg + '장 포함)' : '') + '. 보내면 ' + OUR_NAME + '가 확인하고 보통 1영업일 안에 사이트에 반영합니다.</p>' +
        '<label>보내는 분<input id="es-who" value="' + esc(saved.who || '이상훈') + '"></label><label>연락처<input id="es-tel" value="' + esc(saved.tel || '') + '" placeholder="010-0000-0000"></label>' +
        '<label>남길 말 (선택)<textarea id="es-memo" placeholder="예: 대문 첫 문장 바꿨고, 물놀이 사진 두 장 교체했어요"></textarea></label>' +
      ...
      dlg.querySelector('.send2').addEventListener('click', function () {
        ...
        try { localStorage.setItem('hs-edit-who', JSON.stringify(w)); } catch (e) {}
  ...
  function sendMail(pl, cb) {
    ...
    var msg = '[행사ON 홈페이지 수정 요청]\n보낸 분: ' + pl.who + ' / ' + pl.tel + '\n' + (pl.memo ? '남긴 말: ' + pl.memo + '\n' : '') + '\n' + summary(pl) + '\n\n[JSON — tools/apply_edits.py 에 그대로 넣으면 반영됩니다]\n' + JSON.stringify(light);
    fetch(SEND_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ name: '행사ON / ' + pl.who, phone: pl.tel, email: '', service: '행사ON 홈페이지 수정 요청', message: msg, page: location.href, at: pl.at }) })
      .then(function (r) { return r.ok; }).then(function (ok) { cb(!!ok); }).catch(function () { cb(false); });
  }
  function shareFile(pl, quiet) {
    var name = '행사ON-수정요청-' + pl.at.slice(0, 16).replace(/[-:T]/g, '') + '.json';
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [f] })) { navigator.share({ files: [f], title: '행사ON 홈페이지 수정 파일' }).then(done).catch(function () { download(blob, name); done(); }); return; }
    } catch (e) {}
  ...
  function style() {
    ...
    s.textContent = '.es-e{outline:2px dashed #E0A100;outline-offset:3px;cursor:text;border-radius:2px;min-width:1em}.es-e:hover,.es-e:focus{outline:2px solid #111;background:rgba(255,232,92,.22)}' +
      ...
      '#es-dlg .row{display:flex;gap:8px;flex-wrap:wrap}#es-dlg .b{border:0;border-radius:8px;padding:10px 14px;font:inherit;font-weight:700;cursor:pointer;background:#eee}#es-dlg .send2{background:#0E1B2E;color:#fff}#es-dlg .s{margin:0;font-size:13px;color:#1F7A4D}' +
