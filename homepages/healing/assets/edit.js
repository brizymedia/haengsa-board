...
(function () {
  ...
  try {
    ...
    if (m) { var v = decodeURIComponent(m[1]); if (v === 'off') sessionStorage.removeItem('es-edit-key'); else sessionStorage.setItem('es-edit-key', v); }
    key = sessionStorage.getItem('es-edit-key') || '';
  } catch (e) { key = m ? decodeURIComponent(m[1]) : ''; }
  if (!key || key === 'off' || sha256(key) !== hash) return;

  /* ══ 편집 모드 ══ */
  var SKIP = '#gal,#pfGrid,#boardBlog,#boardFiles,.marq,.lb,.done,.quick,#es-editbar,#es-dlg,form,#tabs';
  ...
  function db(cb) {
    var rq = indexedDB.open('es-edit', 1);
  ...
  function openDialog() {
    loadAll(function (all) {
      ...
      var saved = {}; try { saved = JSON.parse(localStorage.getItem('es-edit-who') || '{}'); } catch (e) {}
      dlg.innerHTML = '<div class="box"><h3>수정 내용 보내기</h3><p class="d">바뀐 곳 <b>' + n + '</b>개' + (nImg ? ' (사진 ' + nImg + '장 포함)' : '') + '. 보내면 ' + OUR_NAME + '가 확인하고 보통 1영업일 안에 사이트에 반영합니다.</p>' +
        '<label>보내는 분<input id="es-who" value="' + esc(saved.who || '이현호') + '"></label><label>연락처<input id="es-tel" value="' + esc(saved.tel || '') + '" placeholder="010-0000-0000"></label>' +
      ...
      dlg.querySelector('.send2').addEventListener('click', function () {
        ...
        try { localStorage.setItem('es-edit-who', JSON.stringify(w)); } catch (e) {}
  ...
  function sendMail(pl, cb) {
    ...
    var msg = '[힐링엔터테인먼트 홈페이지 수정 요청]\n보낸 분: ' + pl.who + ' / ' + pl.tel + '\n' + (pl.memo ? '남긴 말: ' + pl.memo + '\n' : '') + '\n' + summary(pl) + '\n\n[JSON — tools/apply_edits.py 에 그대로 넣으면 반영됩니다]\n' + JSON.stringify(light);
    fetch(SEND_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ name: '힐링엔터테인먼트 / ' + pl.who, phone: pl.tel, email: '', service: '힐링엔터테인먼트 홈페이지 수정 요청', message: msg, page: location.href, at: pl.at }) })
      .then(function (r) { return r.ok; }).then(function (ok) { cb(!!ok); }).catch(function () { cb(false); });
  }
  function shareFile(pl, quiet) {
    var name = '힐링엔터테인먼트-수정요청-' + pl.at.slice(0, 16).replace(/[-:T]/g, '') + '.json';
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [f] })) { navigator.share({ files: [f], title: '힐링엔터테인먼트 홈페이지 수정 파일' }).then(done).catch(function () { download(blob, name); done(); }); return; }
    } catch (e) {}
