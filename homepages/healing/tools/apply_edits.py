# -*- coding: utf-8 -*-
"""
사장님 수정 요청 반영기 — 힐링엔터테인먼트 홈페이지

사장님이 사이트에서 ?edit=열쇠 로 고친 내용은 두 갈래로 온다.
  1) 큰길브리지 문의 메일 「힐링엔터테인먼트 홈페이지 수정 요청」 — 본문 끝에 [JSON] 블록 (글 수정)
  2) 수정 파일 힐링엔터테인먼트-수정요청-YYYYMMDDHHMM.json — 카톡 · 메일 첨부 (글 + 사진)
...
PAGES = ('index.html', 'about.html', 'service.html', 'portfolio.html', 'video.html', 'notice.html', 'contact.html')
APPJS = os.path.join(SITE, 'assets', 'app.js')  # 공지 글(NOTICES)은 페이지가 아니라 여기 있다
    ...
    for page, pg in pl.get('pages', {}).items():
        ...
        for t in pg.get('texts', []):
            ...
            if r: s = s2; n += 1; print('  [%s] %s: %s → %s' % (page, r, strip(t['old'])[:30], strip(t['new'])[:40]))
                ...
                # 공지 · 자료실 글은 app.js 의 NOTICES · FILES 에 들어 있다
                js = open(APPJS, encoding='utf-8').read(); js2, r2 = apply_text(js, t)
                if r2: open(APPJS, 'w', encoding='utf-8').write(js2); print('  [app.js] %s: %s → %s' % (r2, strip(t['old'])[:30], strip(t['new'])[:40]))
