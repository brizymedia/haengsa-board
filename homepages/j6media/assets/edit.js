...
(function () {
  ...
  var meta = document.querySelector('meta[name="es-edit"]');
  if (!meta) return;
  ...
  try {
    ...
    if (m) { var v = decodeURIComponent(m[1]); if (v === 'off') sessionStorage.removeItem('j6-edit-key'); else sessionStorage.setItem('j6-edit-key', v); }
    key = sessionStorage.getItem('j6-edit-key') || '';
  } catch (e) { key = m ? decodeURIComponent(m[1]) : ''; }
  if (!key || key === 'off' || sha256(key) !== hash) return;

  /* ══ 편집 모드 ══ */
  var SKIP = '#gal,#pfGrid,#blogList,#noticeList,#noticeBoard,#fileBoard,#litQuote,.marquee,.lb,#formDone,.quick,#es-editbar,#es-dlg,form';
  ...
  function db(cb) {
    var rq = indexedDB.open('j6-edit', 1);
  ...
  function openDialog() {
    loadAll(function (all) {
      ...
      var saved = {}; try { saved = JSON.parse(localStorage.getItem('j6-edit-who') || '{}'); } catch (e) {}
      dlg.innerHTML = '<div class="box"><h3>수정 내용 보내기</h3><p class="d">바뀐 곳 <b>' + n + '</b>개' + (nImg ? ' (사진 ' + nImg + '장 포함)' : '') + '. 보내면 ' + OUR_NAME + '가 확인하고 보통 1영업일 안에 사이트에 반영합니다.</p>' +
        '<label>보내는 분<input id="es-who" value="' + esc(saved.who || '김호진') + '"></label><label>연락처<input id="es-tel" value="' + esc(saved.tel || '') + '" placeholder="010-0000-0000"></label>' +
      ...
      dlg.querySelector('.send2').addEventListener('click', function () {
        ...
        try { localStorage.setItem('j6-edit-who', JSON.stringify(w)); } catch (e) {}
  ...
  function sendMail(pl, cb) {
    ...
    var msg = '[제이식스미디어 홈페이지 수정 요청]\n보낸 분: ' + pl.who + ' / ' + pl.tel + '\n' + (pl.memo ? '남긴 말: ' + pl.memo + '\n' : '') + '\n' + summary(pl) + '\n\n[JSON — tools/apply_edits.py 에 그대로 넣으면 반영됩니다]\n' + JSON.stringify(light);
    fetch(SEND_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ name: '제이식스미디어 / ' + pl.who, phone: pl.tel, email: '', service: '제이식스미디어 홈페이지 수정 요청', message: msg, page: location.href, at: pl.at }) })
      .then(function (r) { return r.ok; }).then(function (ok) { cb(!!ok); }).catch(function () { cb(false); });
  }
  function shareFile(pl, quiet) {
    var name = '제이식스미디어-수정요청-' + pl.at.slice(0, 16).replace(/[-:T]/g, '') + '.json';
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [f] })) { navigator.share({ files: [f], title: '제이식스미디어 홈페이지 수정 파일' }).then(done).catch(function () { download(blob, name); done(); }); return; }
    } catch (e) {}
  ...
  function style() {
    ...
    s.textContent = '.es-e{outline:2px dashed #E0A100;outline-offset:3px;cursor:text;border-radius:2px;min-width:1em}.es-e:hover,.es-e:focus{outline:2px solid #111;background:rgba(255,232,92,.22)}' +
      ...
      '#es-dlg .row{display:flex;gap:8px;flex-wrap:wrap}#es-dlg .b{border:0;border-radius:8px;padding:10px 14px;font:inherit;font-weight:700;cursor:pointer;background:#eee}#es-dlg .send2{background:#E58A2B;color:#fff}#es-dlg .s{margin:0;font-size:13px;color:#1F7A4D}' +
